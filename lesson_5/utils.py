...
def show_multiple_augmentations(original_img, augmented_imgs, titles):
    ...
    # Конвертируем оригинальное изображение в тензор
    orig_tensor = to_tensor(original_img)  # Добавлено преобразование PIL -> Tensor
    orig_resized = resize_transform(orig_tensor)  # Применяем к тензору
    ...
    # Обработка аугментированных изображений (уже тензоры)
    for i, (aug_img, title) in enumerate(zip(augmented_imgs, titles)):
        aug_resized = resize_transform(aug_img)  # Уже тензор
        aug_np = aug_resized.numpy().transpose(1, 2, 0)
        aug_np = np.clip(aug_np, 0, 1)
